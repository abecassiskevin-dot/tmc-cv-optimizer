...
- Format JSON strict uniquement"""
    ...
    def analyze_cv_matching(self, parsed_cv: Dict[str, Any], jd_text: str) -> Dict[str, Any]:
        """ÉTAPE 1 - Analyse et scoring uniquement (pas de génération CV)"""
        import time
        
        print(f"🔍 Analyse matching (ÉTAPE 1)...", flush=True)
        start_time = time.time()
        
        try:
            client = self._get_anthropic_client()
            
            # Reconstruire le CV en texte
            cv_text = f"""
...
"""
            ...
            # PROMPT ANALYSE UNIQUEMENT
            prompt = f"""🎯 ANALYSE DE MATCHING PONDÉRÉE - SCORING UNIQUEMENT

⚠️ MISSION: Tu dois UNIQUEMENT analyser et scorer le CV contre la JD.
NE GÉNÈRE PAS le CV enrichi - seulement l'analyse.

🔴 PRINCIPE FONDAMENTAL:
- Tu es un SYSTÈME D'ÉVALUATION AUTOMATISÉ
- Pour le MÊME CV et la MÊME JD → EXACTEMENT le même score
- Méthodologie MATHÉMATIQUE et REPRODUCTIBLE
...
Bonus:
- +20% si dans TITRE du poste
- +15% si "Required", "Must have", "Essential"
- +10% si mentionné dans les 3 premières lignes
- +5% par occurrence au-delà de 3

Poids_Final = min(Poids_Base + Bonus, 50%)

RÈGLES STRICTES:
- Stack technique principal: 30-50%
- Compétences secondaires: 15-25%
- Compétences tertiaires: 5-15%
- Soft skills: MAXIMUM 10%
- TOTAL poids = EXACTEMENT 100%

ÉTAPE 2 - SCORER CHAQUE DOMAINE:

📐 FORMULE:
• 0%: ZÉRO mention, stack incompatible
• 10%: Technologie proche
• 25%: 1 mention, formation seulement
• 40%: 1-2 mentions + 1 projet + <1 an
• 60%: 3-4 mentions + 2 projets + 1-2 ans
• 80%: 5+ mentions + 3+ projets + 3+ ans
• 100%: 7+ mentions + expertise senior

Ajustements OBLIGATOIRES:
• Stack incompatible → 0%
• Aucune expérience pratique → MAX 30%
• Expérience <1 an → MAX 50%

Score_Domaine = (Score_Brut × Poids_Domaine) / 100

🔴 VÉRIFICATION:
Somme_Scores = Σ(tous les Score_Domaine)
Le score_matching FINAL = somme EXACTE des scores

JSON STRICT (sans markdown):
{{
  "domaines_analyses": [
    {{
      "domaine": "ALWAYS in English (ex: .NET Stack, Cloud Azure)",
      "poids": 40,
      "score": 15,
      "score_max": 40,
      "commentaire": "ALWAYS in English - 30-50 words with ❌/⚠️/✅",
      "match": "incompatible|partiel|bon|excellent"
    }}
  ],
  "score_matching": 45,
  "synthese_matching": "ALWAYS in English - Summary 2-3 sentences",
  "points_forts": ["ALWAYS in English: strength 1", "ALWAYS in English: strength 2"]
}}

🌍 CRITICAL: domaines_analyses, synthese_matching, points_forts MUST be in ENGLISH
...
Réponds UNIQUEMENT avec du JSON pur."""
            ...
            print(f">>> Calling Claude API for analysis...", flush=True)
            response = client.messages.create(
                model="claude-sonnet-4-5-20250929",
                max_tokens=4000,
                timeout=300.0,
                ...
                try:
                    ...
                    if attempt < 2:
                        fix_response = client.messages.create(
                            ...
                            max_tokens=4000,
                            timeout=60.0,
                            messages=[{"role": "user", "content": f"Fix JSON: {response_text}"}]
                        )
                        response_text = fix_response.content[0].text.strip()
                        for prefix in ['```json', '```']:
                            if response_text.startswith(prefix):
                                response_text = response_text[len(prefix):]
                        if response_text.endswith('```'):
            ...
            cost = round((tokens_in/1_000_000)*3.0 + (tokens_out/1_000_000)*15.0, 4)
            ...
            print(f"✅ Analyse terminée! ⏱️{processing_time}s 📊{total_tokens:,} tokens 💰${cost}")
            print(f"   Score: {analysis.get('score_matching', 0)}/100")
            print(f"   Domaines: {len(analysis.get('domaines_analyses', []))}")
...
Return the corrected JSON directly:"""
    ...
    def generate_cv_from_analysis(self, parsed_cv: Dict[str, Any], jd_text: str, analysis_result: Dict[str, Any], language: str = "French") -> Dict[str, Any]:
        """ÉTAPE 2 - Génération CV enrichi basée sur analyse (OPTION C HYBRIDE - ~1500 tokens)"""
        import time
        
        print(f"✨ Génération CV (ÉTAPE 2 - HYBRIDE)...", flush=True)
        print(f"   Score analyse: {analysis_result.get('score_matching', 0)}/100")
        ...
        try:
            ...
            # Reconstruire CV texte
            cv_text = f"""PROFIL: {parsed_cv.get('profil_resume', '')}
TITRE: {parsed_cv.get('titre_professionnel', '')}
COMPÉTENCES: {chr(10).join(['- ' + c for c in parsed_cv.get('competences', [])])}
EXPÉRIENCES:"""
            for exp in parsed_cv.get('experiences', []):
                cv_text += f"\n{exp.get('periode', '')} | {exp.get('entreprise', '')} | {exp.get('poste', '')}\n"
                for resp in exp.get('responsabilites', []):
            ...
            # Top technologies de l'analyse
            domaines = analysis_result.get('domaines_analyses', [])
            top_tech = [d.get('domaine', '') for d in sorted(domaines, key=lambda x: x.get('score', 0), reverse=True)[:5]]
            top_tech_str = ', '.join(top_tech) if top_tech else "technologies JD"
            
            # PROMPT HYBRIDE compact
            prompt = f"""Tu as DÉJÀ analysé ce CV contre la JD.

ANALYSE (NE PAS REFAIRE):
- Score: {analysis_result.get('score_matching', 0)}/100  
- Domaines: {len(domaines)} pondérés
- Top techs: {top_tech_str}

⚠️ LANGUE {language.upper()}: 100% contenu en {language}

MISSION: GÉNÈRE CV ENRICHI (pas scoring)

PRIORITÉS:
1. PROFIL: Narratif 5-6 lignes en {language}, intégrer {top_tech_str}
2. COMPÉTENCES: Catégories JD, desc 100-150 chars, **2-3 techs** gras
3. EXPÉRIENCES: Bullets 1 ligne, "Action **Tech1** avec **Tech2** résultats"
4. MOTS-CLÉS: 15-20 TECHNOLOGIES (pas verbes)

JSON STRICT:
{{
  "titre_professionnel_enrichi": "TITRE 3-5 mots {language}",
  "profil_enrichi": "Paragraphe 5-6 lignes {language} **techs**",
  "mots_cles_a_mettre_en_gras": ["Tech1", "Tech2"],
  "competences_enrichies": {{"Cat1": ["**Tech**: desc 100-150 chars"]}},
  "experiences_enrichies": [{{
    "periode": "2020-2023",
    "entreprise": "X",
    "poste": "Y",
    "responsabilites": ["Action **Tech** résultats"],
...
CV: {cv_text}"""
            ...
            # Fusionner avec analyse
            enriched['score_matching'] = analysis_result.get('score_matching', 0)
            enriched['domaines_analyses'] = analysis_result.get('domaines_analyses', [])
            enriched['synthese_matching'] = analysis_result.get('synthese_matching', '')
            enriched['points_forts'] = analysis_result.get('points_forts', [])
            
            metadata_step1 = analysis_result.get('_metadata', {})
            enriched['_metadata'] = {
                'step1_time': metadata_step1.get('processing_time_seconds', 0),
                'step1_tokens': metadata_step1.get('total_tokens', 0),
                'step1_cost': metadata_step1.get('estimated_cost_usd', 0),
                'step2_time': processing_time,
                'step2_tokens': total_tokens,
                'step2_cost': cost,
                'total_time': metadata_step1.get('processing_time_seconds', 0) + processing_time,
                'total_tokens': metadata_step1.get('total_tokens', 0) + total_tokens,
                'total_cost_usd': round(metadata_step1.get('estimated_cost_usd', 0) + cost, 4)
            }
            
            print(f"✅ CV généré! ⏱️{processing_time}s 📊{total_tokens:,} tokens 💰${cost}")
            print(f"   📊 TOTAL (2 étapes): {enriched['_metadata']['total_tokens']:,} tokens")
            print(f"   💰 COÛT TOTAL: ${enriched['_metadata']['total_cost_usd']}")
            
            return enriched
            
        except Exception as e:
            print(f">>> ERROR CV generation: {repr(e)}", flush=True)
            return {}
